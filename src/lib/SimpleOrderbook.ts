...
export class SimpleOrderbook {
  ...
  constructor() {
    // Sort ascending
    this.asks = new RBTree((a: PriceLevel, b: PriceLevel) =>
      a.price.gt(b.price) ? 1 : a.price.eq(b.price) ? 0 : -1
    );
    // Sort descending
    this.bids = new RBTree((a: PriceLevel, b: PriceLevel) =>
      a.price.gt(b.price) ? -1 : a.price.eq(b.price) ? 0 : 1
    );
  ...
  public async run(marketId: string) {
    ...
    this.watcher.subscribe("orderbook", [marketId]);
  }
